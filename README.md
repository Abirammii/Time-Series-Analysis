...
-The project is structured into two key notebooks:
  - 1.Data_cleaning functions as the initial notebook, where SQL is used to load raw data, apply cleaning procedures, handle null values, correct data types, and consolidate all tables into a unified ```master_dataaa```.
  - 2.sales_forecasting_timeseries serves as the subsequent notebook, where multiple time series forecasting methods are explored. This includes baseline models, classical statistical approaches, and machine learning–based techniques to evaluate and compare forecasting performance.
